class Zookeeper < ActiveRecord::Base
  ...
  QUERY = "
    ...
    from
      ...
      (select z2.id, CAST(sum(if((c3.status = 0 or c3.status = 2), 1, 0)) AS SIGNED) as controller_offline_node, count(c3.id) as controller_total from zookeepers z2 left join blur_controllers c3 on (z2.id = c3.zookeeper_id) group by z2.id) c,
      (select z3.id, CAST(sum(if((s2.status = 0 or s2.status = 2), 1, 0)) AS SIGNED) as shard_offline_node, count(s2.id) as shard_total from zookeepers z3 left join clusters c4 on (z3.id = c4.zookeeper_id) left join blur_shards s2 on (c4.id = s2.cluster_id) group by z3.id) s,
