...
//								System.out.println("Start time:" + blurQueryStatus.getQuery().get);
								jdbc.update("insert into blur_queries (query_string, cpu_time, real_time, complete, interrupted, running, uuid, created_at, table_name, super_query_on, facets, start, fetch_num, pre_filters, post_filters, selector_column_families, selector_columns) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", 
