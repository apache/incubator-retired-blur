...
public class ZookeeperDatabaseConnection implements ZookeeperDatabaseInterface, ControllerDatabaseInterface, TableDatabaseInterface,
  ...
  public int markOfflineControllers(List<String> onlineControllers, int zookeeperId) {
    if (onlineControllers.isEmpty()) {
      ...
      return this.jdbc.update(
          "update blur_controllers set status=0, updated_at=? where status!=0 and node_name not in ('" + StringUtils.join(onlineControllers, "','")
              + "') and zookeeper_id = ?", TimeHelper.now().getTime(), zookeeperId);
    }
  }
  ...
  public void markOfflineTables(List<String> onlineTables, int clusterId) {
    if (onlineTables.isEmpty()) {
      ...
      this.jdbc.update(
          "update blur_tables set status=0, updated_at=? where status!=0 and cluster_id=? and table_name not in ('"
  ...
  public List<String> getRecentOfflineControllerNames(int amount) {
    return this.jdbc.queryForList("select node_name from blur_controllers where status=0 order by updated_at limit 0, " + amount, String.class);
  }
}
