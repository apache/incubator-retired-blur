...
public class AgentMailerTest {
	...
	private void assertProperMessageSent(String type, String name, Wiser server) throws MessagingException, IOException {
	    ...
	    assertEquals("Blur Console has received notice that " + type + " [" + name + "] has recently gone offline, if this was expected please ignore this email.", StringUtils.trim((String) email.getMimeMessage().getContent()));
	}
