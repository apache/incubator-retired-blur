...
/**
 * This class, similar to {@link org.apache.lucene.index.IndexWriter}, has no
 * locking mechanism.
 * <p>
 * {@link org.apache.lucene.store.instantiated.InstantiatedIndexReader} is
 * navigating the same instances in memory as this writer is updating so
 * searchers active while you are committing are bound to throw exceptions.
 * <p>
...
public class InstantiatedIndexWriter implements Closeable {
   ...
   */
  private Map<InstantiatedDocument, Map<FieldSetting, Map<String /* text */, TermDocumentInformationFactory>>> termDocumentInformationFactoryByDocument = new LinkedHashMap<InstantiatedDocument, Map<FieldSetting, Map<String /* text */, TermDocumentInformationFactory>>>(
      2000);
  ...
  /**
   * The sweetspot for this implementation is somewhere around 2500 at 2K text
   * large documents.
   * <p/>
   * Benchmark output:
   * 
  ...
  /**
   * If non-null, information about merges and a message when maxFieldLength is
   * reached could be printed to this -- currently not yet implemented.
   */
   ...
   */
  public int docCount() {
    // todo: not certain. see
    // http://www.nabble.com/IndexWriter.docCount-tf3128882.html#a8669483
    return index.getDocumentsByNumber().length /*
                                                * -
  ...
  public void commit() throws IOException {
    ...
    for (Map.Entry<InstantiatedDocument, Map<FieldSetting, Map<String /* text */, TermDocumentInformationFactory>>> eDocumentTermDocInfoByTermTextAndField : termDocumentInformationFactoryByDocument
        .entrySet()) {
      ...
      for (Map.Entry<FieldSetting, Map<String /* text */, TermDocumentInformationFactory>> eFieldTermDocInfoFactoriesByTermText : eDocumentTermDocInfoByTermTextAndField.getValue()
          .entrySet()) {
      ...
      Map<InstantiatedTerm, InstantiatedTermDocumentInformation> informationByTermOfCurrentDocument = new HashMap<InstantiatedTerm, InstantiatedTermDocumentInformation>(
          termsInDocument);
      ...
      // terms...
      for (Map.Entry<FieldSetting, Map<String /* text */, TermDocumentInformationFactory>> eFieldSetting_TermDocInfoFactoriesByTermText : eDocumentTermDocInfoByTermTextAndField
          .getValue().entrySet()) {
        ...
        for (Map.Entry<String /* text */, TermDocumentInformationFactory> eTermText_TermDocInfoFactory : eFieldSetting_TermDocInfoFactoriesByTermText.getValue().entrySet()) {
          ...
          InstantiatedTermDocumentInformation info = new InstantiatedTermDocumentInformation(term, document, /*
          ...
          // todo optimize, this should be cached and updated to array in
          // batches rather than appending the array once for every position!
    ...
    for (InstantiatedTerm term : dirtyTerms) {
      // todo optimize, i believe this is useless, that the natural order is
      // document number?
  ...
  /**
   * Tokenizes a document and adds it to the buffer. Try to do all calculations
   * in this method rather than in commit, as this is a non locking method.
   * Remember, this index implementation expects unlimited memory for maximum
   * speed.
   * 
   * @param document
   * @param analyzer
  ...
  protected void addDocument(InstantiatedDocument document, Analyzer analyzer) throws IOException {
    ...
    for (Fieldable field : document.getDocument().getFields()) {
      ...
      // todo: fixme: multiple fields with the same name does not mean field
      // boost += more boost.
      ...
      if (field.isIndexed()) {
        ...
        if (field.isTokenized()) {
          ...
          while (tokenStream.incrementToken()) {
            // TODO: this is a simple workaround to still work with tokens, not
            // very effective, but as far as I know, this writer should get
            // removed soon:
            ...
            for (Iterator<AttributeImpl> atts = tokenStream.getAttributeImplsIterator(); atts.hasNext();) {
              ...
              try {
                ...
                // this may fail to copy some attributes, if a special combined
                // AttributeImpl is used, that
                // implements basic attributes supported by Token and also other
                // customized ones in one class.
              }
            }
    ...
    for (Map.Entry<Fieldable, LinkedList<Token>> eField_Tokens : tokensByField.entrySet()) {
      ...
      Map<String, TermDocumentInformationFactory> termDocumentInformationFactoryByTermText = termDocumentInformationFactoryByTermTextAndFieldSetting.get(fieldSettingsByFieldName
          .get(eField_Tokens.getKey().name()));
      ...
      if (fieldSetting.position > 0) {
        // todo what if no analyzer set, multiple fields with same name and
        // index without tokenization?
  ...
  public Set<Term> getUnflushedDeletions() {
    return unflushedDeletions;
  }
