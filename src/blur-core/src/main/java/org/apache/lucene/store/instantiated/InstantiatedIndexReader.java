...
public class InstantiatedIndexReader extends IndexReader {
  ...
  public boolean isDeleted(int n) {
    BitVector deletedDocuments = index.getDeletedDocuments();
    if (deletedDocuments != null && n < deletedDocuments.size() && deletedDocuments.get(n)) {
        return true;
    }
    if (uncommittedDeletedDocuments != null && n < uncommittedDeletedDocuments.size() && uncommittedDeletedDocuments.get(n)) {
        return true;
    }
    return false;
  }
